"""
Python module for SNPSeeker.Version 1.
...
An example for using this module:
    you can set values to the parameters (learnrateW, learnratePsi, dropout_rate, minP, thr_p, thr_rp, thr_t, thr_t2, zero)
		SNPSeeker(GSMT, sample, character, GeneSnpMTGene, GeneSnpMTSnp, data, ResultFolder,
	...
	or you can use the default parameters:
        SNPSeeker(GSMT, sample, character, GeneSnpMT, GeneSnpMTGene, GeneSnpMTSnp, data, ResultFolder, 
...
def multilogrank_test(Samindx, newsample, newdata, ST, thr_p):
    #ST sample times
    """ return the union of two lists """
    count=0
    # count the number of significant test during ST sampling.
    for itr in range(ST):
        T = newdata["OS_MONTHS"]
        E = [0] * len(newsample)
        ...
        for m in range(0, len(newsample)):
            if (m not in Samindx):
                Samindex2.append(m)
        saminx = random.sample(Samindex2, len(Samindx))
        if(len(saminx)>2):
...
def multilogrank_test4(samp,MT,data,temchar,char,thr_p):
    # use to validate genes in a bicluster
    """ return the union of two lists """
    rdsampleindex = random.sample(range(MT.shape[1]), 100)
    # random select 100 sample no reptive
    TestMT = MT[:, rdsampleindex]
    # Test set expression matrix
    ...
    for inx in range(0, len(Testsample)):
        ...
        temSNP = char[ix]  # SNPs in sample inx
        temcount = len(np.intersect1d(temSNP, temchar))
        if (temcount > (len(temchar) * 0.5)):
            # if (temcount >= len(temchar)*0.5):  # this sample include all the SNPs in the bicluster
            E3[inx] = 1
            # seperate all samples into two groups 1 if the sample include all SNPs in the bicluster and 0 otherwise
        else:
...
def data_Process(rawpath,TMuppath,TMdnpath): # generate discritized datasets
    #rawpath = "/home/mamingrui/data/data_cBioportal1_median_Zscores_1.txt"  # processed median-zscore matrix
    rawM = np.loadtxt(rawpath, np.float32)
    TMup = np.zeros(rawM.shape, dtype=np.float32)
    # up-regulated genes
    TMdn = np.zeros(rawM.shape, dtype=np.float32)
    # down-regulated genes
    for r in range(rawM.shape[0]):
        TMr1 = np.quantile(rawM[r, :], 0.75)
        TMr2 = np.quantile(rawM[r, :], 0.25)
        for j in range(rawM.shape[1]):
            if (rawM[r, j] > TMr1 and rawM[r, j] > 0):
            # only keep real high-up regulated genes
                TMup[r, j] = rawM[r, j]
    ...
    f.close()
    return TMup_1, TMdn_1


def SNPSeeker(GSMT, sample, character, GeneSnpMTGene, GeneSnpMTSnp, data, ResultFolder, TMup_1, n_hidden, n_iter,
             ITt,**kwargs):
    # main code of SNPSeeker
    if not kwargs:
        learnrateW = 0.1
        learnratePsi = 0.10
        dropout_rate = 0.1
        ...
        learnrateW = kwargs['learnrateW']
        learnratePsi = kwargs['learnrateW']
        dropout_rate = kwargs['dropout_rate']
        minP = kwargs['minP']
        thr_p = kwargs['thr_p']
        thr_rp = kwargs['thr_rp']
        thr_t = kwargs['thr_t']
        thr_t2 = kwargs['thr_t2']
        zero = kwargs['zero']
        GPUID = kwargs['GPUID']
    ...
    MT = TMup_1
    Bicp = []
    # use to storage p value
    Bicp2 = []
    # use to storage p2 value
    num = 0
    ST = 100
    # sampling times
    E=7
    # E=-ln(10-3), for bicluster statistical evaluation
    for y in range(ITt):
        print('Iter times: ', y)
        ...
        W, P, Wout = train_rfn(newMT.transpose(), n_hidden, n_iter, learnrateW, learnratePsi, minP, dropout_rate,
                               activation="relu", gpu_id=GPUID)
        H = np.maximum(0, np.dot(Wout, newMT))
        for index in range(0, len(W)):
            temmax = np.max(W[index])
            if (temmax > 0.06):
            # if max value in W <0 then no bicluster elements are found
                Snpindx = []
                # storege SNP location in a bicluster
                ...
                if (len(Snpindx) > 2):
                # at least two chars included in a bicluster
                    Samindx = []
                    # storege SNP location in a bicluster
                    temmax2 = np.max(H[index])
                    # max value in H
                    ...
                    if (len(Samindx) > 2):
                    # at least two samples included
                        BicM1 = newMT[Snpindx, :]
                        BicM = BicM1[:, Samindx]
                        Rato = (float(np.count_nonzero(BicM)) / (len(Snpindx) * len(Samindx)))
                        # none zero ratio in bicluster detected, quality of bicluster
                        Ratoall = (float(np.count_nonzero(newMT)) / (len(newMT) * len(newMT[0])))
                        # none-zero ratio of the input matrix
                        Cmnk = float(np.count_nonzero(BicM)) - (len(Snpindx) * len(Samindx) * Ratoall) - math.sqrt(
                            (3 * E) * (len(Snpindx) * len(Samindx) * Ratoall))
                        # statistical significance
                        if (Rato > zero and Cmnk>0):
                        # bicluster fullfill threshold
                            ...
                            if (rp > thr_rp):
                                ...
                                if (rp2 < thr_p):
                                # this part need be changed, as rp2 is not a ST times sampling results
                                    ...
                                    for inx1 in range(0, len(temchar)):
                                        Sloc = MGene.tolist().index(temchar[inx1])
                                        # get the location of SNPs in temchar
                                        Gloc = GSMT[:, Sloc].tolist().index(1)
                                        # get the location of SNP related gene
                                        TGene = MSnp[Gloc]
